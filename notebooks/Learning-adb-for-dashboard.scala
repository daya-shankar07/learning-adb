...
// MAGIC 
// MAGIC case class Order(orderId:Int, orderDate:String, customerId:Int, amount:Int)
// MAGIC case class Customer(id:BigInt, name:String, email:String)

// COMMAND ----------

val customers = spark.read.json("/mnt/salesdata2/customers.json").as[Customer]
val orders = spark.read.option("inferSchema",true).option("header",true).option("sep",",")
	.csv("/mnt/salesdata2/orders.csv").as[Order]
...
// MAGIC %md
// MAGIC #Sales Performace Dashboard
